...
% Generate correlation matrix for each users
ks = randi(50,1,K+1);
for idx_k=1:K+1
   k=ks(idx_k);
   R =  functionRlocalscattering(M,varphiRange(k),ASD,antennaSpacing);
   Rsqrt_k(:,:,idx_k) = sqrt(R);
end
...
%     Channels(i,:,:) = normalize(Rsqrt*uncorrelatedRealizations1(:,:,i),'scale'); %  normalize by its standard deviation
    for k=1:K
        Channels(i,:,k) = normalize( Rsqrt_k(:,:,k) * uncorrelatedRealizations1(:,k,i), 'scale'); 
    end
...
uncorrelatedRealizations2 = (randn(M,K+1,Attack_Data)+1i*randn(M,K+1,Attack_Data))/sqrt(2);
Channels_test=zeros(Attack_Data,M,K);
Y_real=zeros(Attack_Data,M,K);

for j=1:Attack_Data
    for k=1:K
        Channels_test(j,:,k) = normalize( Rsqrt_k(:,:,k) * uncorrelatedRealizations2(:,k,j), 'scale'); 
    end
...
%     Channels_test(j,:,:) = normalize(Rsqrt*uncorrelatedRealizations2(:,:,j),'scale'); %  normalize by its standard deviation
    ...
    Channels_att(j,:,1) = normalize( Rsqrt_k(:,:,K+1) * uncorrelatedRealizations2(:,K+1,j), 'scale'); 
%     Channels_att(j,:,1) = normalize(Rsqrt*uncorrelatedRealizations2(:,1,j),'scale');
    CH_att(:,1)=Channels_att(j,:,1);
    Y_att(j,:,:)=CH_att(:,1)*Pilot(1,:);
